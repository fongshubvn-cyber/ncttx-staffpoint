...
export const Option1SummaryView: React.FC<Option1SummaryViewProps> = ({
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <div className="bg-white rounded-3xl p-3.5 px-5 shadow-sm border border-slate-200/80 relative z-30">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          
          {/* Pillar Selector Button */}
          <div className="relative flex items-center gap-2.5 min-w-0">
            <span className="hidden xl:inline-flex items-center gap-1.5 text-xs font-extrabold uppercase tracking-wider text-slate-400 shrink-0">
              <User className="w-3.5 h-3.5 text-emerald-600" />
              <span>Nhân sự ({visibleStaffList.length}):</span>
            </span>

            <button
              type="button"
              onClick={() => setIsStaffDropdownOpen(!isStaffDropdownOpen)}
              className="inline-flex items-center gap-3 p-2 px-3.5 rounded-2xl bg-emerald-50/70 hover:bg-emerald-100/70 border border-emerald-300/60 shadow-xs hover:shadow transition-all duration-200 group text-left max-w-full cursor-pointer"
            >
              <div className="w-8 h-8 rounded-xl bg-gradient-to-tr from-emerald-600 to-teal-600 text-white font-extrabold flex items-center justify-center text-xs shadow-sm shrink-0">
                {staff.name.substring(0, 1)}
              </div>

              <div className="min-w-0 flex items-center gap-2 flex-wrap sm:flex-nowrap">
                <span className="font-extrabold text-sm text-slate-900 group-hover:text-emerald-800 transition-colors whitespace-nowrap">
                  {staff.name}
                </span>
                <span className="font-mono text-emerald-800 font-extrabold bg-white/90 px-2 py-0.5 rounded-md text-xs border border-emerald-300/60 shrink-0">
                  {staff.id}
                </span>
                <span className="text-xs text-slate-400 shrink-0">•</span>
                <span className="text-xs text-slate-600 font-semibold truncate max-w-[180px] sm:max-w-[280px]">
                  {staff.department || staff.line}
                </span>
              </div>

              <div className="flex items-center gap-1.5 ml-2 pl-2.5 border-l border-emerald-300/60 shrink-0">
                <span className="text-xs font-bold text-emerald-800 bg-white/80 px-2.5 py-0.5 rounded-lg hidden sm:inline-block border border-emerald-200/60">
                  Đổi nhân sự
                </span>
                <div className="p-1 rounded-lg bg-white/80 border border-emerald-300/60 text-emerald-700 group-hover:bg-white transition-all">
                  <ChevronDown className={`w-3.5 h-3.5 transition-transform duration-200 ${isStaffDropdownOpen ? 'rotate-180' : ''}`} />
                </div>
              </div>
            </button>
            ...
            {isStaffDropdownOpen && (
              <>
                ...
                ></div>
                <div className="absolute left-0 top-full mt-2 z-50 bg-white/95 backdrop-blur-xl border border-slate-200/90 shadow-2xl rounded-3xl p-3 animate-in fade-in slide-in-from-top-2 duration-200 max-h-96 min-w-[320px] sm:min-w-[420px] flex flex-col">
                          ...
                          >
                            <div className="flex items-center gap-2.5 min-w-0">
                              ...
                              <div className="min-w-0 flex items-center gap-2 flex-wrap">
                                <span className="truncate font-extrabold text-xs">{s.name}</span>
                                <span className={`font-mono text-[10px] font-bold px-1.5 py-0.2 rounded shrink-0 ${
                                  isSelected ? 'bg-white/20 text-white' : 'bg-slate-100 text-slate-600'
                                }`}>
                                  {s.id}
                                </span>
                                <span className={`text-[11px] truncate ${isSelected ? 'text-emerald-100' : 'text-slate-500'}`}>
                                  • {s.department || s.line}
                                </span>
                              </div>
                            </div>
